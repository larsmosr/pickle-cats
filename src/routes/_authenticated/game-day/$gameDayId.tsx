    ...
    <div className="flex-1 flex flex-col pb-24">
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="flex items-center justify-center gap-3">
          <Label htmlFor="mode" className={cn(!isDoubles && 'text-muted-foreground')}>
            Singles
          </Label>
        ...
        {matchup && matchup.team1.length > 0 && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-center text-sm text-muted-foreground">Game {games.length + 1}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  {matchup.team1.map((player) => (
                    <button
                      key={player._id}
                      type="button"
                      className="flex w-full items-center gap-2 p-2  pr-4 rounded-lg bg-muted/50 cursor-pointer hover:bg-muted"
              ...
              <Button className="w-full" size="lg" disabled={!canSubmit || isSubmitting} onClick={handleSubmitGame}>
        ...
        {matchup && matchup.sittingOut.length > 0 && (
          <div>
            <h3 className="text-sm font-medium text-muted-foreground mb-2">Sitting Out</h3>
            <div className="flex gap-2 flex-wrap">
              {matchup.sittingOut.map((player) => (
                <div key={player._id} className="flex items-center gap-2 px-3 py-2 rounded-full bg-muted/50">
                  <Avatar size="sm">
                    <AvatarImage src={player.avatarUrl} />
                    <AvatarFallback>{player.name.charAt(0)}</AvatarFallback>
                  </Avatar>
        ...
        {games.length > 0 && (
          <div>
            <h3 className="text-sm font-medium text-muted-foreground mb-2">Previous Games</h3>
            <div className="space-y-2">
      ...
      {games.length > 0 && (
        <AlertDialog open={completeDialogOpen} onOpenChange={setCompleteDialogOpen}>
          <AlertDialogTrigger
            render={
              <Button className="fixed bottom-6 left-1/2 -translate-x-1/2 shadow-lg" size="lg" variant="secondary">
                View Summary
              </Button>
            }
          />
